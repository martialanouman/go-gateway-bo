...
tableau des rôles par défaut. `make generate` ne lit que les deux contrats OpenAPI — client Go de
l'API Admin, serveur Go et types TS du BFF, les trois que `check-generated` compare : la dérivation
du catalogue vers le TypeScript arrive avec step-006, et rien ne garde cet écart d'ici là. Si une
**route client** est ajoutée : l'arbre de routes régénéré **et commité** (`make check-routes` le
vérifie).
