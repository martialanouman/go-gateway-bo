...
- **Un contrôle interdit est désactivé et expliqué**, jamais silencieusement masqué. Le rendu
  conditionnel reste un confort : la garde est côté serveur, invariant (c).
...
rien ; le chargement à froid y est un sixième *moment*, pas un sixième état.
...
- **Bout en bout (Playwright)** — cinq parcours seulement, **contre le binaire**, jamais `vite dev` :
...
créer le fichier sous `web/src/routes/`, régénérer l'arbre, **commiter `routeTree.gen.ts`**. Un TS
engendré non régénéré fait rougir `check-generated`.
