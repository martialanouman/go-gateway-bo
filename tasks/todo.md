...
| Le plancher de `registerRowCount` est à **60** pour un registre qui porte **80** lignes. | Un quart du registre peut disparaître sans un rougissement — le défaut même que le plancher raconte avoir corrigé en passant de 45 à 60. Remesuré le 19/09/2026 avec la logique de la porte. C'est un échantillon prélevé sur la donnée qu'il garde, donc il dérive à chaque ligne ajoutée. | step-187 |
| `TestUneLectureDeSessionResteDansSonBudget` échoue environ **une fois sur quatre**, sur `main` comme ailleurs. | Le budget de 1,50 est posé au sommet de la dispersion naturelle. Mesuré le 19/09/2026, machine au repos : `main` rend 1,26 · 1,20 · 1,40 · 1,50 (échec), la branche de step-038 rend 1,31 · 1,35 · 1,56 (échec). Une porte rouge au hasard finit désactivée, et elle rend `make check` non concluant. Le remède n'est **pas** de relever le seuil — c'est la mesure qu'il faut rendre robuste (médiane de plusieurs relevés). | step-187 |
