...
`web/CLAUDE.md`, la charte §08 — et **aucun n'existe en composant** : `HomeScreen` réécrit son état
vide à la main, et il est le seul écran du produit.
...
- **Une seule animation en boucle dans tout le système** : la pulsation de 1,8 s du point vivant. Les
  squelettes scintillent à 1,4 s, les compteurs temps réel changent de valeur sans transition, et
  `prefers-reduced-motion` désactive l'ensemble.
...
- [ ] `HomeScreen` consomme l'état vide livré ici et n'en garde aucune copie locale
...
Le `Banner` → la step qui l'affiche. La pile de toasts **branchée sur le temps réel** → step-043 : ce
qui est livré ici est le composant, pas sa source. L'`AppShell` qui les héberge → step-040.
